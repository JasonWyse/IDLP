function [learned_matrix_cell, best_parameter_array, evaluation_result] = Learn(input_parameter_cell, matrix_cell_train, ...,
    ...
    indice_set = input_parameter_cell{3,1};
    ...
    evaluation_result = zeros(indistct_num+length(indice_set),length(alpha_set)*length(beta_set));
    i = 1;
    for lambda = alpha_set
        for eta = beta_set
                ...
                evaluation_result(end-indistct_num+1:end,i) = [lambda;eta];
                i = i+1;
                toc;
        end
    end
