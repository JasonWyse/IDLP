function [learned_matrix_cell, best_parameter_array, evaluation_result] = Learn(input_parameter_cell, matrix_cell_train, ...,
    ...
    evaluation_result = zeros(indistct_num+length(indice_set),length(beta_set));
    i = 1;
    for beta = beta_set
        tic;
        max_ites = 20;
        cv_train_parameter_cell = {[beta]; max_ites;...,
        ...
        evaluation_result(end-indistct_num+1:end,i) = [beta];
        i = i+1;
        toc;
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%% cross-validation to choose best parameters  %%%%%%%%%%%%%%%%%%%%%%%%%%
